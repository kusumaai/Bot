...
class RiskConfig:
    ...
    risk_factor: Decimal
    ratchet_thresholds: List[float]
    ratchet_lock_ins: List[float]
    ...
    def from_yaml(cls, path: str) -> 'RiskConfig':
        ...
        thresholds = config.get('ratchet_thresholds', [2, 4, 6])
        lock_ins = config.get('ratchet_lock_ins', [1, 2, 3])
        ...
        return cls(
            ...
            max_drawdown=Decimal(str(config.get('max_drawdown_pct', '10'))) / Decimal('100'),
