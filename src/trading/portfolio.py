...
from src.utils.error_handler import handle_error_async, handle_error
...
class PortfolioManager:
    ...
    async def update_current_balance(self) -> None:
        ...
        if self.db_queries is None:
            self.logger.warning("db_queries not provided in context; skipping balance update.")
            return
        try:
            balance = await self.db_queries.get_account_balance()
            self.current_balance = Decimal(balance)
        except Exception as e:
            await handle_error_async(e, "PortfolioManager.update_current_balance", self.logger)
    ...
    async def add_position(self, symbol: str, size: Decimal, entry_price: Decimal) -> bool:
        """
        ...
        async with self._async_lock:
            try:
                ...
                if len(self.positions) >= self.risk_limits.get('max_positions', 10):
                    self.logger.warning(f"Max positions limit reached: {self.risk_limits.get('max_positions', 10)}")
                    return False

                position_value = size * entry_price
                total_value = self.calculate_portfolio_value()
                if total_value > Decimal('0') and (position_value / total_value) > self.risk_limits.get('max_position_size', Decimal('0.1')):
    ...
    async def update_position_price(self, symbol: str, current_price: Decimal) -> None:
        ...
        async with self._async_lock:
            try:
                ...
                position.unrealized_pnl = position.size * (new_price - position.entry_price)
    ...
    def calculate_percentage_change(self, old_value: Decimal, new_value: Decimal) -> Decimal:
        """
        Calculate the percentage change between two values.
        ...
        try:
            ...
            handle_error(e, "PortfolioManager.calculate_percentage_change", logger=self.logger)
            return Decimal('0')
