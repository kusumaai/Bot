...
def calculate_log_returns(prices: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        asyncio.create_task(
            handle_error_async(
                e, "math.calculate_log_returns", logging.getLogger(__name__)
            )
        )
        return np.array([])
...
def calculate_trend_probability(
    predicted_return: float, ema_short: float, ema_long: float, base_prob: float = 0.4
) -> float:
    """
...
def calculate_expected_value_and_targets(
    ...
    """
    try:
        win_target = current_price * (
            Decimal(str(np.exp(float(predicted_return)))) - Decimal("1")
        )
        loss_target = current_price * stop_loss_pct
        ev = up_prob * (win_target - transaction_cost) - (Decimal("1") - up_prob) * (
            loss_target + transaction_cost
        )
        return ev, win_target, loss_target
    except Exception as e:
        handle_error(e, "math.calculate_expected_value_and_targets")
        return Decimal("0"), Decimal("0"), Decimal("0")


# calculate the kelly fraction for position sizing
def calculate_kelly_fraction(
    up_prob: Decimal, win_target: Decimal, loss_target: Decimal
) -> Decimal:
...
class MathHandler:
    ...
    def calculate_expected_value(
        self, probability: Decimal, win_amount: Decimal, loss_amount: Decimal
    ) -> Decimal:
        """
        ...
        """
        try:
            return (win_amount - loss_amount) * probability - loss_amount * (
    ...
    # calculate the kelly fraction for position sizing
    def calculate_kelly_fraction(
        self, win_amount: Decimal, loss_amount: Decimal
    ) -> Decimal:
        """
    ...
    def calculate_position_size(
        ...
        try:
            if volatility <= Decimal("0") or current_price <= Decimal("0"):
                return Decimal("0")
            base_size = (account_balance * Decimal("0.1")) / (
...
def calculate_expected_value(
    probability: Union[str, float, Decimal],
    potential_profit: Union[str, float, Decimal],
    potential_loss: Union[str, float, Decimal],
) -> Decimal:
    """
    Calculate the expected value of a trade.
    ...
    Args:
        ...
        potential_profit: Potential profit amount
    ...
    Raises:
        MathError: If calculation fails
    """
    try:
        prob = nh.to_decimal(str(probability))
        profit = nh.to_decimal(str(potential_profit))
        loss = nh.to_decimal(str(potential_loss))

        if not (Decimal("0") <= prob <= Decimal("1")):
            raise MathError("Probability must be between 0 and 1")

        return (prob * profit) - ((Decimal("1") - prob) * loss)

    except InvalidOperation as e:
        logger.error(f"Invalid operation in expected value calculation: {e}")
        raise MathError(f"Invalid operation in expected value calculation: {e}")
    except Exception as e:
