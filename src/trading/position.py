...
from risk.manager import PositionInfo
from utils.numeric_handler import NumericHandler
from utils.exceptions import PositionError, InvalidOrderError, DatabaseError  # Optionally, PositionUpdateError if defined
...
#position class that represents a position in the portfolio
@dataclass
class Position:
    ...
    # Additional fields from the old version
    ...
    # Additional optional fields expected by new getters
        ...
        async with self._lock:
            try:
                if current_price <= Decimal('0'):
                    raise PositionError("Current price must be positive.")
                ...
                raise PositionError(f"Invalid operation: {e}") from e
            except Exception as e:
                logging.getLogger(__name__).error(f"Unexpected error during position update: {e}")
                raise PositionError(f"Unexpected error: {e}") from e
    ...
    async def update_price(self, new_price: Decimal) -> None:
        ...
        try:
            ...
            await handle_error_async(e, "Position.update_price", logging.getLogger(__name__))
            raise PositionError(f"Error updating price: {e}")
    ...
    async def close(self, close_price: Decimal) -> None:
        ...
        try:
            ...
            raise PositionError(f"Error closing position: {e}")
    ...
    async def update_market_state(self, market_state: MarketState) -> None:
        ...
        try:
            ...
            await handle_error_async(e, "Position.update_market_state", logging.getLogger(__name__))
            raise
...
class PositionManager:
    ...
    async def get_position_by_timestamp(self, timestamp: int) -> Dict[str, Position]:
        ...
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.timestamp == timestamp}

    async def get_position_by_side(self, side: str) -> Dict[str, Position]:
        """
        Get all positions by their side.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.side == side}

    async def get_position_by_strategy(self, strategy: str) -> Dict[str, Position]:
        """
        Get all positions by their strategy.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.strategy == strategy}

    async def get_position_by_entry_price(self, entry_price: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their entry price.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.entry_price == entry_price}

    async def get_position_by_size(self, size: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their size.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.size == size}

    async def get_position_by_status(self, status: str) -> Dict[str, Position]:
        """
        Get all positions by their status.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if ('CLOSED' if pos.closed else 'OPEN') == status}

    async def get_position_by_pnl(self, pnl: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their PnL.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.realized_pnl == pnl}

    async def get_position_by_unrealized_pnl(self, unrealized_pnl: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their unrealized PnL.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.unrealized_pnl == unrealized_pnl}

    async def get_position_by_realized_pnl(self, realized_pnl: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their realized PnL.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.realized_pnl == realized_pnl}

    async def get_position_by_pnl_percentage(self, pnl_percentage: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their PnL percentage.
        (Assumes each position has an attribute or computed property 'pnl_percentage'.)
        """
        return {symbol: pos for symbol, pos in self.positions.items() if getattr(pos, 'pnl_percentage', None) == pnl_percentage}

    async def get_position_by_stop_loss(self, stop_loss: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their stop loss.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.stop_loss == stop_loss}

    async def get_position_by_take_profit(self, take_profit: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their take profit.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.take_profit == take_profit}

    async def get_position_by_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Position]:
        """
        Get all positions by their metadata.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.metadata == metadata}

    async def get_position_by_current_price(self, current_price: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their current price.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.current_price == current_price}

    async def get_position_by_sentiment(self, sentiment: str) -> Dict[str, Position]:
        """
        Get all positions by their sentiment.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.sentiment == sentiment}

    async def get_position_by_sentiment_score(self, sentiment_score: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their sentiment score.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.sentiment_score == sentiment_score}

    async def get_position_by_sentiment_magnitude(self, sentiment_magnitude: Decimal) -> Dict[str, Position]:
        """
        Get all positions by their sentiment magnitude.
        """
        return {symbol: pos for symbol, pos in self.positions.items() if pos.sentiment_magnitude == sentiment_magnitude}
