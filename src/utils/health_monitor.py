...
class HealthMonitor:
    ...
    async def _check_exchange(self) -> bool:
        ...
        try:
            ...
            if self.exchange_interface and hasattr(self.exchange_interface.exchange, 'ping'):
                try:
                ...
                if len(self.latency_history[component]) >= 10:
                    ...
                    if comp:
                        ...
                        if latency > median * float(self.degradation_threshold):
                            self.logger.warning(f"{component} showing performance degradation: "
        ...
        async with self._component_lock:
            try:
                ...
                if component == 'database':
                    ...
                    metrics = self.check_system_resources()
                    healthy = all(metrics[k] for k in ['memory_healthy', 'disk_healthy', 'cpu_healthy'])
                    response_time = 0.0
                else:
                    ...
                    if check_method:
                        ...
                        self.logger.warning(f"No check method found for component: {component}")
                        return
                ...
                await handle_error_async(e, "HealthMonitor.monitor_component_health", self.logger)
                self.update_component(
                    component=component,
                    healthy=False,
    ...
    async def is_system_healthy(self) -> bool:
        ...
        try:
            ...
            self.components['system'].status = all([
                db_healthy,
                exchange_healthy,
                self.check_system_resources()['healthy']
            ])
            ...
            if self.ctx.db_connection:
                async with self.ctx.db_connection.pool.acquire() as conn:
                    async with conn.cursor() as cursor:
                        ...
                        await cursor.execute("SELECT COUNT(*) FROM models WHERE status = 'active'")
                        model_count = await cursor.fetchone()
                        readiness['models'] = model_count[0] > 0 if model_count else False
                        
                        # Check for GA data
                        await cursor.execute("SELECT COUNT(*) FROM genetic_algorithms WHERE status = 'complete'")
                        ...
                        readiness['market_data'] = data_count[0] > 0 if data_count else False
                        
                        readiness['database'] = True  # Database exists and can be queried
