...
def validate_positive_decimal(value: Decimal, field_name: str, allow_zero: bool = False) -> Optional[str]:
    if allow_zero:
        if value < Decimal('0'):
...
# Domain-specific types for the bot types
...
class Position(Validatable):
    ...
    def validate(self) -> ValidationResult:
        ...
        if self.direction not in ['long', 'short']:
            return ValidationResult(is_valid=False, error_message="Direction must be 'long' or 'short'")
...
class RiskLimits(Validatable):
    ...
    def validate(self) -> ValidationResult:
        if self.max_position_size > Decimal('0.5'):
            return ValidationResult(is_valid=False, error_message="max_position_size cannot exceed 0.5 (50%)")
        err = validate_positive_decimal(self.min_position_size, "min_position_size", allow_zero=True)
        if err:
            return ValidationResult(is_valid=False, error_message=err)
        if self.max_daily_loss > Decimal('0.03'):
            return ValidationResult(is_valid=False, error_message="max_daily_loss cannot exceed 0.03 (3%)")
        if self.max_drawdown > Decimal('0.2'):
            return ValidationResult(is_valid=False, error_message="max_drawdown cannot exceed 0.2 (20%)")
        if self.emergency_stop_pct > Decimal('0.15'):
            return ValidationResult(is_valid=False, error_message="emergency_stop_pct cannot exceed 0.15 (15%)")
        if self.risk_factor > Decimal('0.02'):
            return ValidationResult(is_valid=False, error_message="risk_factor cannot exceed 0.02 (2%)")
        if self.max_correlation > Decimal('0.7'):
            return ValidationResult(is_valid=False, error_message="max_correlation cannot exceed 0.7 (70%)")
        if self.max_sector_exposure > Decimal('0.3'):
            return ValidationResult(is_valid=False, error_message="max_sector_exposure cannot exceed 0.3 (30%)")
        if self.max_volatility > Decimal('0.4'):
            return ValidationResult(is_valid=False, error_message="max_volatility cannot exceed 0.4 (40%)")
        if self.max_leverage > Decimal('3'):
            return ValidationResult(is_valid=False, error_message="max_leverage cannot exceed 3x")
        if self.kelly_scaling > Decimal('0.5'):
            return ValidationResult(is_valid=False, error_message="kelly_scaling cannot exceed 0.5 (50%)")
        return ValidationResult(is_valid=True)
