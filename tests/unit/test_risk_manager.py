...
from utils.exceptions import MathError, RiskCalculationError, RiskError
...
async def test_calculate_position_size(risk_limits, db_queries, logger):
    ...
    expected_size = (Decimal("10000") * (Decimal("1") / Decimal("100"))).quantize(
        Decimal("0.0001")
    )  # 100 / 50000 = 0.0020
...
async def test_validate_trade_within_limits(risk_manager):
    ...
    is_valid, error = await risk_manager.validate_trade(
        symbol="BTC/USDT", side="buy", amount=Decimal("0.05"), price=Decimal("50000")
    )
...
async def test_validate_trade_below_min_size(risk_manager):
    ...
    is_valid, error = await risk_manager.validate_trade(
        symbol="BTC/USDT", side="buy", amount=Decimal("0.005"), price=Decimal("50000")
    )
...
@pytest.mark.asyncio
async def test_validate_trade_exceeds_max_positions(
    risk_manager, mock_portfolio_manager
):
    ...
    is_valid, error = await risk_manager.validate_trade(
        symbol="ETH/USDT", side="sell", amount=Decimal("0.05"), price=Decimal("3000")
    )
    assert is_valid is False
