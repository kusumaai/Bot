...
async def test_execute_trade_exchange_error(exchange_interface):
    ...
    exchange_interface.exchange_manager.exchange.create_order.side_effect = (
...
async def test_cancel_trade_success(exchange_interface):
    ...
    exchange_interface.exchange_manager.exchange.close_order.assert_awaited_once_with(
        "order123"
    )
...
async def test_fetch_candles_success(exchange_interface):
    ...
    exchange_interface.exchange_manager.exchange.fetch_ohlcv.assert_awaited_once_with(
        "BTC/USDT", "15m", limit=2
    )
...
async def test_fetch_candles_exchange_error(exchange_interface):
    ...
    exchange_interface.exchange_manager.exchange.fetch_ohlcv.side_effect = (
