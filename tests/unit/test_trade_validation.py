#! /usr/bin/env python3
# tests/unit/test_trade_validation.py
"""
Module: tests.unit
Provides unit testing functionality for the trade validation module.
"""
import logging
...
def test_validate_trade_with_high_correlation(market_data_validation):
    ...
    with pytest.raises(
        ValidationError, match="Correlation for ETH/USDT exceeds maximum allowed"
    ):
...
def test_validate_trade_with_low_liquidity(market_data_validation):
    ...
    with pytest.raises(
        ValidationError, match="Liquidity for BTC/USDT is below minimum required"
    ):
