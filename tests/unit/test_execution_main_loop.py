...
async def test_main_loop_execute_cycle_success(main_loop):
    ...
    main_loop.exchange_interface.fetch_market_data.return_value = {'BTC/USDT': {'price': Decimal('50000')}}
    main_loop.risk_manager.validate_market_conditions.return_value = True
    main_loop.exchange_interface.place_trade.return_value = {'success': True, 'order_id': 'order006'}
    ...
    main_loop.risk_manager.validate_market_conditions.assert_awaited_once_with({'BTC/USDT': {'price': Decimal('50000')}})
    main_loop.exchange_interface.place_trade.assert_awaited_once()
    main_loop.db_queries.log_trade.assert_awaited_once_with('order006', {'symbol': 'BTC/USDT', 'side': 'buy', 'amount': Decimal('1'), 'price': Decimal('50000')})
