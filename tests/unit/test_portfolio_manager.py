...
def risk_limits():
    ...
    return RiskLimits.from_config(
        {"max_positions": 5, "max_position_size": "0.5", "max_leverage": "3.0"}
    )
...
def portfolio_manager(risk_limits, mock_logger):
    ...
    ctx.config = {"portfolio": {"initial_balance": "10000"}}
...
def test_get_total_exposure(portfolio_manager):
    ...
    expected_exposure = (Decimal("0.1") * Decimal("50000")) + (
        Decimal("0.2") * Decimal("3000")
    )  # 5000 + 600 = 5600
