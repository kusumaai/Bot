#! /usr/bin/env python3
# tests/unit/test_numeric_handler.py
"""
Module: tests.unit
...
def test_to_decimal_invalid_values(numeric_handler, caplog):
    ...
    with caplog.at_level(logging.ERROR):
        ...
        assert (
            "Failed to convert value to Decimal: invalid - [<class 'decimal.ConversionSyntax'>]"
            in caplog.text
        )

        assert numeric_handler.to_decimal(None) is None
        assert (
            "Failed to convert value to Decimal: None - [<class 'decimal.ConversionSyntax'>]"
            in caplog.text
        )
...
def test_convert_to_decimal_invalid(numeric_handler):
    with pytest.raises(
        MathError, match="Invalid operation during conversion to Decimal:"
    ):
