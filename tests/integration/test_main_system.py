...
def mock_exchange_interface():
    ...
    interface.place_trade = AsyncMock(
        return_value={"success": True, "order_id": "order130"}
    )
...
async def test_full_trading_cycle_trade_failure(main_loop):
    ...
    with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        ...
        main_loop.exchange_interface.place_trade.assert_awaited_once()
        main_loop.db_queries.log_trade.assert_not_awaited()
        main_loop.logger.error.assert_called_with(
            "Trade placement failed: Trade Failed"
        )
        mock_sleep.assert_awaited_once_with(1)
...
async def test_full_trading_cycle_fetch_market_data_failure(main_loop):
    ...
    main_loop.exchange_interface.fetch_market_data.side_effect = Exception(
    ...
    with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        ...
        main_loop.db_queries.log_trade.assert_not_awaited()
        main_loop.logger.error.assert_called_with(
            "Error in MainLoop.execute_cycle: Market Data Fetch Error"
        )
        mock_sleep.assert_awaited_once_with(1)
